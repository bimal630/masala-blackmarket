...
RegisterNetEvent('masala-blackmarket:server:UpdateShopItems', function(shop, itemData, amount)
    ...
    TriggerClientEvent('masala-blackmarket:client:SetShopItems', -1, shop, Config.Locations[shop]["products"])
end)

RegisterNetEvent('masala-blackmarket:server:RestockShopItems', function(shop)
    if Config.Locations[shop]["products"] ~= nil then
        ...
        TriggerClientEvent('masala-blackmarket:client:RestockShopItems', -1, shop, randAmount)
    end
end)

QBCore.Functions.CreateCallback('masala-blackmarket:server:getLicenseStatus', function(source, cb)
    local src = source
...
RegisterNetEvent('masala-blackmarket:server:sellChips', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
